...
public final class ProductSkuIdCodec {

    public static final String BASE62_PATTERN = "^[0-9A-Za-z]{1,22}$";

    private static final int SKU_ID_BYTES = 16;
    ...
    public static byte[] fromBase62(String value) {
        if (value == null || !value.matches(BASE62_PATTERN)) {
            throw new IllegalArgumentException("SKU ID must be Base62.");
        }
        ...
        if (raw.length > SKU_ID_BYTES + 1 || (raw.length == SKU_ID_BYTES + 1 && raw[0] != 0)) {
            throw new IllegalArgumentException("SKU ID exceeds 16 bytes.");
        }
        byte[] normalized = new byte[SKU_ID_BYTES];
        int sourceStart = raw.length > SKU_ID_BYTES ? raw.length - SKU_ID_BYTES : 0;
        int copyLength = raw.length - sourceStart;
        System.arraycopy(raw, sourceStart, normalized, SKU_ID_BYTES - copyLength, copyLength);
        return normalized;
    }
    ...
    private static byte[] fromHex(String value) {
        ...
        if (!normalized.matches("^[0-9A-Fa-f]{32}$")) {
            throw new IllegalArgumentException("SKU ID hex must be 16 bytes.");
        }
        return HexFormat.of().parseHex(normalized);
    }

    private static byte[] normalizeBytes(byte[] bytes) {
        if (bytes == null || bytes.length != SKU_ID_BYTES) {
            throw new IllegalArgumentException("SKU ID must be 16 bytes.");
        }
        return bytes;
    }
}
