...
CREATE TABLE IF NOT EXISTS ipv4_reputation_profile (
    -- 自增主键，避免用 IP 字符串直接作为主键
    id BIGSERIAL PRIMARY KEY,

    -- IPv4 字符串仍然作为业务唯一键，供 ON CONFLICT (ip) 使用
...
ALTER TABLE ipv4_reputation_profile
...
CREATE INDEX IF NOT EXISTS idx_ipv4_reputation_profile_admin_order
    ON ipv4_reputation_profile (
        current_score ASC,
        is_tor DESC,
        is_proxy DESC,
        is_vpn DESC,
...
COMMENT ON COLUMN ipv4_reputation_profile.id IS '自增主键';
COMMENT ON COLUMN ipv4_reputation_profile.ip IS 'IPv4 地址，业务唯一键';
...
COMMENT ON COLUMN ipv4_reputation_profile.region IS '一级行政区 / 州 / 省，例如 California / Guangdong';
COMMENT ON COLUMN ipv4_reputation_profile.city IS '城市名称，例如 Los Angeles / Shenzhen';
