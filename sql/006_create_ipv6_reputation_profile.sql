-- ============================================
-- 文件名：006_create_ipv6_reputation_profile.sql
-- 说明：IPv6 信誉画像最小版表
-- 约定：
-- 1. 首次出现 IPv6 时调用外部 API 填充
...
CREATE TABLE IF NOT EXISTS ipv6_reputation_profile (
    ...
    CONSTRAINT ck_ipv6_reputation_profile_ip_type
...
CREATE INDEX IF NOT EXISTS idx_ipv6_reputation_profile_expires_at
    ON ipv6_reputation_profile (expires_at);

CREATE INDEX IF NOT EXISTS idx_ipv6_reputation_profile_asn
    ON ipv6_reputation_profile (asn);

CREATE INDEX IF NOT EXISTS idx_ipv6_reputation_profile_country
...
COMMENT ON COLUMN ipv6_reputation_profile.is_tor IS '是否 Tor';
COMMENT ON COLUMN ipv6_reputation_profile.provider_score IS '第三方供应商原始分数（如果有）';
COMMENT ON COLUMN ipv6_reputation_profile.reference_score IS '参考分数：纯粹基于第三方 API 结果，不考虑行为分';
COMMENT ON COLUMN ipv6_reputation_profile.base_score IS '首次出现时根据第三方情报换算得到的初始分，可作为系统首次入库分';
COMMENT ON COLUMN ipv6_reputation_profile.current_score IS '当前行为分，基于第三方风险字段换算得到';
COMMENT ON COLUMN ipv6_reputation_profile.source_provider IS '第三方数据来源';
COMMENT ON COLUMN ipv6_reputation_profile.raw_json IS '第三方 API 原始返回 JSON';
COMMENT ON COLUMN ipv6_reputation_profile.first_seen_at IS '首次出现时间';
COMMENT ON COLUMN ipv6_reputation_profile.last_seen_at IS '最近一次出现时间';
COMMENT ON COLUMN ipv6_reputation_profile.queried_at IS '最近一次调用外部 API 查询时间';
COMMENT ON COLUMN ipv6_reputation_profile.expires_at IS '缓存过期时间';
