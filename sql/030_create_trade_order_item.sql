...
-- 4. SKU 名称、编码、规格、图片和价格保存下单时快照；
-- 5. 即使商品后续修改，订单明细仍保留下单当时的商品信息。
-- ============================================
...
COMMENT ON TABLE trade_order_item IS '订单明细表：一行代表订单中的一个 SKU 快照。下单后即使商品名称、价格、图片、规格发生变化，订单明细仍保留下单当时的商品信息。';
...
COMMENT ON COLUMN trade_order_item.line_amount_yuan IS '该明细行金额，单位：元，通常等于 sale_price_yuan * quantity';
