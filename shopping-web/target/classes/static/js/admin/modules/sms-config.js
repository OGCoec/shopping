(function (root) {
  ...
  async function load() {
    ...
    try {
      ...
      setStatus(nodes, error.message || "读取阿里云短信服务配置失败。", "error");
    } finally {
  ...
  async function save() {
    ...
    if (!accessKeyId.trim() && !accessKeySecret.trim()) {
      setStatus(nodes, "请至少填写一个需要修改的短信服务配置值。", "error");
      return;
    }
    if (nodes.saveButton) {
    ...
    setStatus(nodes, "正在保存阿里云短信服务 Windows 系统环境变量...");
    try {
      ...
      setStatus(nodes, `已保存到 ${target}，重启应用后短信客户端生效。`, "ok");
    } catch (error) {
      setStatus(nodes, error.message || "保存阿里云短信服务配置失败。", "error");
    } finally {
      if (nodes.saveButton) {
  ...
  root.AdminSmsConfigModule = { mount };
})(window);
