...
public class OrderExpireService {
    ...
    public boolean finalizeClosing(String orderNo) {
        ...
        OffsetDateTime now = OffsetDateTime.now();
        FinalizeClosingResult result = transactionTemplate.execute(status -> finalizeClosingInTransaction(normalizedOrderNo, now));
        if (result == null) {
            return false;
        }
        if (result.retry()) {
    ...
    private boolean startPersistedClosing(String orderNo, OffsetDateTime now, OffsetDateTime closingDeadline) {
        Map<String, Object> row = transactionTemplate.execute(status -> orderMapper.startClosingExpiredOrder(
                orderNo,
                now,
                closingDeadline
        ));
    ...
    private FinalizeClosingResult finalizePersistedOrder(String orderNo, Map<String, Object> persisted, OffsetDateTime now) {
        ...
        releaseResources(updatedUserId, orderNo, itemRows, hasUserCoupon(updated), now);
