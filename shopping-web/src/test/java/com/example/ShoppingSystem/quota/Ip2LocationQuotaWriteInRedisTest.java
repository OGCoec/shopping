...
class Ip2LocationQuotaWriteInRedisTest {
    ...
    @Test
    void shouldWriteQuotaToRedisDb2() {
        ...
        String quotaCount = redisTemplate.opsForValue().get("ip2location:quota:count");
        assertEquals(String.valueOf(REMAINING_QUOTA), quotaValue, "Redis 中应写入剩余额度");
        assertEquals("1", quotaCount, "Redis 中应写入可用 key 总数");
