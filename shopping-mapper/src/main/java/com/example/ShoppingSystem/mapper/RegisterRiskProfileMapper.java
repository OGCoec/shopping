...
public interface RegisterRiskProfileMapper {
    ...
    @Select("""
            ...
            updated AS (
                UPDATE device_risk_profile profile
                SET current_score = CASE
                        WHEN target.should_penalize
                        THEN GREATEST(0, profile.current_score - GREATEST(0, #{penaltyScore}))
                        ELSE profile.current_score
                    END,
                    risk_level = CASE
                        WHEN target.should_penalize
                        THEN CASE
                            WHEN GREATEST(0, profile.current_score - GREATEST(0, #{penaltyScore})) >= 8500 THEN 'L1'
                            WHEN GREATEST(0, profile.current_score - GREATEST(0, #{penaltyScore})) >= 7500 THEN 'L2'
                            WHEN GREATEST(0, profile.current_score - GREATEST(0, #{penaltyScore})) >= 6000 THEN 'L3'
                            WHEN GREATEST(0, profile.current_score - GREATEST(0, #{penaltyScore})) >= 4800 THEN 'L4'
                            WHEN GREATEST(0, profile.current_score - GREATEST(0, #{penaltyScore})) >= 3000 THEN 'L5'
                            ELSE 'L6'
                        END
                    ...
                    last_penalized_ip_transition = CASE
                        WHEN target.should_penalize
                        THEN #{transition}
                        ELSE profile.last_penalized_ip_transition
                    END,
                    last_penalty_at = CASE
                        WHEN target.should_penalize
                        THEN CAST(#{seenAt} AS TIMESTAMPTZ)
                        ELSE profile.last_penalty_at
                    END,
                    last_penalty_score = CASE
                        WHEN target.should_penalize
                        THEN GREATEST(0, #{penaltyScore})
                        ELSE profile.last_penalty_score
                    END,
                    last_penalty_reason = CASE
                        WHEN target.should_penalize
                        THEN #{penaltyReason}
                        ELSE profile.last_penalty_reason
                    END,
                    used_ip_list = CASE
                        WHEN CAST(#{currentIp} AS TEXT) IS NULL OR btrim(CAST(#{currentIp} AS TEXT)) = ''
                        THEN profile.used_ip_list
                        WHEN profile.used_ip_list @> jsonb_build_array(#{currentIp})
                        THEN profile.used_ip_list
                        ELSE profile.used_ip_list || jsonb_build_array(#{currentIp})
                    END,
                    recent_ip_switch_count = CASE
                        WHEN profile.last_login_ip IS NOT NULL
                            AND btrim(profile.last_login_ip) <> ''
                            AND CAST(#{currentIp} AS TEXT) IS NOT NULL
                            AND btrim(CAST(#{currentIp} AS TEXT)) <> ''
                            AND profile.last_login_ip IS DISTINCT FROM #{currentIp}
                        THEN profile.recent_ip_switch_count + 1
                        ELSE profile.recent_ip_switch_count
                    END,
                    updated_at = #{seenAt}
                FROM target
                WHERE profile.id = target.id
                RETURNING profile.id AS device_id,
                          target.score_before AS score_before,
                          profile.current_score AS score_after,
                          target.should_penalize AS should_penalize
            ),
            inserted AS (
                INSERT INTO device_risk_score_event (
                    device_id,
                    score_before,
                    penalty_score,
                    score_after,
                ...
                SELECT device_id,
                       ...
                       #{seenAt}
                FROM updated
                WHERE should_penalize
                  AND score_after < score_before
                  AND btrim(COALESCE(CAST(#{penaltyReason} AS TEXT), '')) <> ''
                RETURNING id
            )
            SELECT (SELECT COUNT(1) FROM updated)::INT
                   + (SELECT COUNT(1) FROM inserted)::INT * 0
            """)
    ...
    @Select("""
            ...
            updated AS (
                UPDATE device_risk_profile profile
                SET current_score = GREATEST(0, profile.current_score - GREATEST(0, #{penaltyScore})),
                    risk_level = CASE
                        WHEN GREATEST(0, profile.current_score - GREATEST(0, #{penaltyScore})) >= 8500 THEN 'L1'
                        WHEN GREATEST(0, profile.current_score - GREATEST(0, #{penaltyScore})) >= 7500 THEN 'L2'
                        WHEN GREATEST(0, profile.current_score - GREATEST(0, #{penaltyScore})) >= 6000 THEN 'L3'
    ...
    @Select("""
            ...
            updated AS (
                ...
                SET current_score = GREATEST(0, profile.current_score - GREATEST(0, #{penaltyScore})),
                    ...
                    last_login_ip = CASE
                        ...
                        THEN profile.last_login_ip
                        ELSE #{clientIp}
                    END,
                    last_ip_seen_at = CASE
                        WHEN btrim(COALESCE(CAST(#{clientIp} AS TEXT), '')) = ''
                        THEN profile.last_ip_seen_at
            ...
            SELECT updated.score_after + inserted_ref.inserted_count::INT * 0
            FROM updated
            LEFT JOIN (SELECT COUNT(1) AS inserted_count FROM inserted) inserted_ref ON TRUE
            LIMIT 1
            """)
    ...
    @Select("""
            ...
            upserted AS (
                ...
                SET current_score = CASE
                    ...
                    END,
                    updated_at = EXCLUDED.updated_at
                RETURNING encode(id, 'hex') AS id_hex,
                          id AS device_id,
                          COALESCE(
                              (SELECT score_before FROM existing),
                              LEAST(10000, #{currentScore} + GREATEST(0, #{lastPenaltyScore}))
                          ) AS score_before,
                          current_score AS score_after,
                          CASE
                              WHEN EXISTS (SELECT 1 FROM existing)
                              THEN (
                                  #{lastPenaltyScore} > 0
                                  AND (SELECT last_login_ip FROM existing) IS NOT NULL
                                  AND btrim((SELECT last_login_ip FROM existing)) <> ''
                                  AND btrim(COALESCE(CAST(#{lastLoginIp} AS TEXT), '')) <> ''
                                  AND (SELECT last_login_ip FROM existing) IS DISTINCT FROM #{lastLoginIp}
                                  AND (SELECT last_penalized_ip_transition FROM existing) IS DISTINCT FROM
                                      ((SELECT last_login_ip FROM existing) || '->' || #{lastLoginIp})
                              )
                              ELSE #{lastPenaltyScore} > 0
                          END AS should_record_event
            ),
            inserted AS (
                INSERT INTO device_risk_score_event (
                    device_id,
                ...
                WHERE should_record_event
                  AND score_after < score_before
                  AND btrim(COALESCE(CAST(#{lastPenaltyReason} AS TEXT), '')) <> ''
                RETURNING id
            )
            SELECT upserted.id_hex
            FROM upserted
            LEFT JOIN (SELECT COUNT(1) AS inserted_count FROM inserted) inserted_ref ON TRUE
            LIMIT 1
            """)
