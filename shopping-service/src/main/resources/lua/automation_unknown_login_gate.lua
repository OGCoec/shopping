...
local device_unknown_key = KEYS[3]
local ip_unknown_key = KEYS[4]
...
if existing_device_ttl_ms > 0 or existing_ip_ttl_ms > 0 then
    ...
    return {1, 'EXISTING_DEVICE_BLOCK', 'EXISTING_IP_BLOCK', retry_after_ms, 0, 0, existing_device_ttl_ms, existing_ip_ttl_ms, 0, 0}
end
...
local ip_count = 0
if device_enabled then
    device_count = incr_window(device_unknown_key, 1800)
end
if ip_enabled then
    ip_count = incr_window(ip_unknown_key, 1800)
end
...
local ip_reason = ''
if ip_enabled then
    if ip_count > tonumber(ARGV[23]) then
        ip_block_seconds = tonumber(ARGV[24])
        ip_penalty = tonumber(ARGV[25])
        ip_reason = ARGV[26]
    elseif ip_count > tonumber(ARGV[19]) then
        ip_block_seconds = tonumber(ARGV[20])
        ip_penalty = tonumber(ARGV[21])
        ip_reason = ARGV[22]
    elseif ip_count > tonumber(ARGV[15]) then
        ip_block_seconds = tonumber(ARGV[16])
        ip_penalty = tonumber(ARGV[17])
        ip_reason = ARGV[18]
    end
end
...
return {blocked, device_reason, ip_reason, retry_after_ms, device_penalty, ip_penalty, device_block_ttl_ms, ip_block_ttl_ms, device_count, ip_count}
