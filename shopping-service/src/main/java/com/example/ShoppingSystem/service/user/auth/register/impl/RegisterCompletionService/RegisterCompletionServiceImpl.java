...
public class RegisterCompletionServiceImpl implements RegisterCompletionService {
    ...
    private final RegisterCoreWriter registerCoreWriter;
    ...
    public RegisterCompletionServiceImpl(StringRedisTemplate stringRedisTemplate,
                                          ...
                                          RegisterCoreWriter registerCoreWriter) {
        ...
        this.registerCoreWriter = registerCoreWriter;
    }
    ...
    public RegisterCompletionResult verifyEmailCodeAndRegister(String email,
        ...
        String userRiskLevel = normalizeUserRiskLevel(riskLevelRaw);
        ...
        // CORE 库身份、档案与 outbox_event 写入由 @TransactionalOutbox 切面在同一 CORE 本地事务内原子提交，事件绝不丢失。
        // RISK 库风控档案与设备风控由 UserRegisteredConsumer 消费 outbox 事件后幂等补写，最终一致。
        registerCoreWriter.writeIdentityAndEnqueue(
                identity,
                resolveUsername(meta.getStr("username"), normalizedEmail),
