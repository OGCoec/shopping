...
import com.example.ShoppingSystem.service.user.auth.passwordreset.PasswordResetCryptoService;
@Service
public class PasswordResetServiceImpl implements PasswordResetService {
    ...
    private final PasswordResetCoreWriter passwordResetCoreWriter;
    ...
    public PasswordResetServiceImpl(UserLoginIdentityMapper userLoginIdentityMapper,
                                    ...
                                    PasswordResetCoreWriter passwordResetCoreWriter) {
        ...
        this.passwordResetCoreWriter = passwordResetCoreWriter;
    }
    ...
    public PasswordResetResult resetByLink(String token,
        ...
        // CORE 库密码写入与 outbox_event 写入放在同一个 CORE 本地事务内，保证原子提交/回滚，事件绝不丢失。
        // RISK 库设备风控成功记录由 PasswordResetSucceededConsumer 消费 outbox 事件后幂等补写，最终一致。
        PasswordResetSucceededMessage resetMessage = buildResetMessage(userId, deviceFingerprint, clientIp);
        boolean updated = passwordResetCoreWriter.updatePasswordAndEnqueue(
                userId, passwords.password(), resetMessage);
        if (!updated) {
    ...
    public PasswordResetResult resetByVerifiedCode(String token,
        ...
        // CORE 库密码写入与 outbox_event 写入放在同一个 CORE 本地事务内，保证原子提交/回滚，事件绝不丢失。
        // RISK 库设备风控成功记录由 PasswordResetSucceededConsumer 消费 outbox 事件后幂等补写，最终一致。
        PasswordResetSucceededMessage resetMessage = buildResetMessage(
                identity.getUserId(), deviceFingerprint, clientIp);
        boolean updated = passwordResetCoreWriter.updatePasswordAndEnqueue(
                identity.getUserId(), passwords.password(), resetMessage);
        if (!updated) {
            return PasswordResetResult.fail("PASSWORD_RESET_FAILED", "Password reset failed.");
        }
        stringRedisTemplate.delete(emailCodeKey(email));
    ...
    private PasswordResetSucceededMessage buildResetMessage(Long userId, String deviceFingerprint, String clientIp) {
        long occurredAtMillis = System.currentTimeMillis();
        String eventId = "pwd-reset-" + userId + "-" + occurredAtMillis;
        return new PasswordResetSucceededMessage(
                eventId,
                userId,
                deviceFingerprint,
                clientIp,
